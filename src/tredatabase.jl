...
"""
function get_datafile_meta(store::DataStore, assetversion::AssetVersion, domain::Domain)::Union{DataFile,Nothing}
...
function get_eav_variables(store::DataStore, datafile::DataFile)::Vector{Variable}
    db = store.lake
    sql = """
    ...
    df = DBInterface.execute(stmt) |> DataFrame
    if nrow(df) == 0
        @warn "No EAV variables found in datafile: $(file_uri_to_path(datafile.storage_uri))"
        return Variable[]
    end
    # variables = Variable[]
    # for row in eachrow(df)
