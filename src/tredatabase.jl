...
function createvariables(conn::DBInterface.Connection)
    ...
    COMMENT ON COLUMN vocabularies.name IS 'Name of the vocabulary, should be unique and comply with xsd:token definition';
    """
    DBInterface.execute(conn, sql)
...
Retrieve a Vocabulary by its name from the datastore.
"""
function get_vocabulary(store::DataStore, name::AbstractString)::Union{Vocabulary,Nothing}
    vocab_id = get_namedkey(store.store, "vocabularies", name, :vocabulary_id)
