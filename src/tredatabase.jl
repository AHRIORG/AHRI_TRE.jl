...
function replace_database(conn::DBInterface.Connection, database::String, user::String, password::String)
    @debug "Recreating database $(database) owned by $(user)"
    try
...
function opendatastore(server::AbstractString, user::AbstractString, password::AbstractString, database::AbstractString,
    ...
    if !isnothing(lake_data) && !isnothing(lake_db)
        ...
        if !isdir(lake_data)
            @debug "Creating lake data directory $(lake_data)"
            mkpath(lake_data)
        end
        ddb = DuckDB.DB()
...
#region Create Datastore Tables
"""
...
function createstudies(conn::DBInterface.Connection)
    ...
    @debug "Created study_types table"
    sql = raw"""
    ...
    """
    DBInterface.execute(conn, sql)
    @debug "Created studies table"
    DBInterface.execute(conn, initstudytypes())
    @debug "Initialized study types"
    sql = raw"""
    ...
    """
    DBInterface.execute(conn, sql)
    @debug "Created study_access table"
    sql = raw"""
...
"""
    ...
    """
    DBInterface.execute(conn, sql)
    @debug "Created study_domains table"
    return nothing
end
...
function createtransformations(conn::DBInterface.Connection)
    ...
    """
    DBInterface.execute(conn, sql)
    @debug "Created transformations table"
    return nothing
end
"""
    createvariables(conn)
...
function createvariables(conn::DBInterface.Connection)
    ...
    """
    DBInterface.execute(conn, sql)
    @debug "Created value_types table"
    sql = raw"""
    CREATE TABLE IF NOT EXISTS vocabularies (
        ...
        name VARCHAR(80) NOT NULL,
        description TEXT,
        CONSTRAINT fk_vocabularies_domain_id FOREIGN KEY (domain_id) REFERENCES domains(domain_id),
        CONSTRAINT uk_vocabularies_domain_name UNIQUE (domain_id, name)
    );
    ...
    COMMENT ON COLUMN vocabularies.domain_id IS 'Domain ID for which the vocabulary is valid, making vocabularies unique within a domain';
    COMMENT ON COLUMN vocabularies.name IS 'Name of the vocabulary, should be unique within the domain and comply with xsd:token definition';
    """
    DBInterface.execute(conn, sql)
    @debug "Created vocabularies table"
    sql = raw"""
    ...
    """
    DBInterface.execute(conn, sql)
    @debug "Created vocabulary_items table"
    sql = raw"""
...
function createassets(conn::DBInterface.Connection)
    ...
    """
    DBInterface.execute(conn, sql)
    @debug "Created asset_versions table"
    ...
    """
    DBInterface.execute(conn, sql)
    @debug "Created trigger to ensure only one latest version per asset"
    ...
    """
    DBInterface.execute(conn, sql)
    @debug "Created trigger to prevent changes to major/minor/patch"
    sql = raw"""
    ...
    """
    DBInterface.execute(conn, sql)
    @debug "Created transformation_inputs table"
    sql = raw"""
    ...
    """
    DBInterface.execute(conn, sql)
    @debug "Created transformation_outputs table"
    sql = raw"""
    ...
    """
    DBInterface.execute(conn, sql)
    @debug "Created dataset_variables table"
    return nothing
end
"""
    createentities(conn)
...
function createentities(conn::DBInterface.Connection)
    ...
    """
    DBInterface.execute(conn, sql)
    @debug "Created entityrelations table"
    sql = raw"""
    ...
    """
    DBInterface.execute(conn, sql)
    @debug "Created entity_instances table"
    sql = raw"""
    ...
    """
    DBInterface.execute(conn, sql)
    @debug "Created relation_instances table"
    sql = raw"""
    ...
    """
    DBInterface.execute(conn, sql)
    @debug "Created data_asset_entities table"
    return nothing
end
...
function createmapping(conn::DBInterface.Connection)
    ...
    """
    DBInterface.execute(conn, sql)
    @debug "Created variable_mapping table"
    return nothing
end
#endregion
"""
...
function upsert_study!(store::DataStore, study::Study)::Study
    ...
    if study.study_id === nothing
        ...
        @debug "Inserting new study: $(study.name)"
        sql = raw"""
        ...
        @debug "Updating existing study: $(study.name) with ID $(study.study_id)"
...
function get_study(store::DataStore, id::UUID)::Union{Study,Nothing}
    ...
    if nrow(df) == 0
        @debug "No study found with ID: $(id)"
        return nothing
    end
    row = df[1, :]
    study = Study(
...
function get_study(store::DataStore, name::AbstractString)::Union{Study,Nothing}
    ...
    if nrow(df) == 0
        @debug "No study found with name: $(name)"
        return nothing
    end
    row = df[1, :]
    study = Study(
...
function get_domain(store::DataStore, name::AbstractString; uri::Union{Nothing,String}=nothing)::Union{Domain,Nothing}
    ...
    if nrow(df) == 0
        @debug "No domain found with name: $(name) and URI: $(uri)"
        return nothing
    end
    row = df[1, :]
    return Domain(
...
function add_study_domain!(store::DataStore, study::Study, domain::Domain)
    ...
    @debug "Added domain $(domain.name) to study $(study.name)"
...
function upsert_entity!(store::DataStore, entity::Entity)::Entity
    ...
    if entity.entity_id === nothing
        @debug "Inserting new entity: $(entity.name) in domain $(entity.domain.name)"
        sql = raw"""
...
function create_asset(store::DataStore, study::Study, name::String, type::String, description::Union{Missing,String}=missing)::Asset
    if !is_ncname(name)
        @debug "Asset name '$name' must be a valid xsd:NCName format"
        name = to_ncname(name)
        @debug "Coerced asset name to '$name'"
    end
...
function add_transformation!(store::DataStore, transformation::Transformation)::Transformation
    db = store.store
    @debug "Inserting new transformation: $(transformation.description)"
...
function add_transformation_output(store::DataStore, transformation_id::Int, version_id::UUID)::Nothing
    db = store.store
    @debug "Adding transformation output for transformation ID $(transformation_id) and version ID $(version_id)"
    sql = raw"""
...
function add_transformation_input(store::DataStore, transformation_id::Int, version_id::UUID)::Nothing
    db = store.store
    @debug "Adding transformation input for transformation ID $(transformation_id) and version ID $(version_id)"
    sql = raw"""
...
"""
    get_vocabulary(store::DataStore, domain_id::Int, name::AbstractString)::Union{Vocabulary,Nothing}

Retrieve a Vocabulary by its domain_id and name from the datastore.
"""
function get_vocabulary(store::DataStore, domain_id::Int, name::AbstractString)::Union{Vocabulary,Nothing}
    db = store.store
    sql = raw"""
        SELECT vocabulary_id FROM vocabularies WHERE domain_id = $1 AND name = $2 LIMIT 1;
    """
    stmt = DBInterface.prepare(db, sql)
    df = DBInterface.execute(stmt, (domain_id, name)) |> DataFrame
    if nrow(df) == 0
        return nothing
    end
    return get_vocabulary(store, Int(df[1, :vocabulary_id]))
end

"""
    get_vocabulary(store::DataStore, name::AbstractString)::Union{Vocabulary,Nothing}

Retrieve a Vocabulary by name alone from the datastore.
Returns the vocabulary only if it exists in exactly one domain.
Throws an error if the vocabulary name exists in multiple domains or doesn't exist.
"""
function get_vocabulary(store::DataStore, name::AbstractString)::Union{Vocabulary,Nothing}
    db = store.store
    sql = raw"""
        SELECT vocabulary_id, domain_id FROM vocabularies WHERE name = $1;
    """
    stmt = DBInterface.prepare(db, sql)
    df = DBInterface.execute(stmt, (name,)) |> DataFrame
    ...
    if nrow(df) == 0
        return nothing
    elseif nrow(df) > 1
        domains = join(df[!, :domain_id], ", ")
        error("Vocabulary name '$name' is not unique; found in multiple domains: $domains. Use get_vocabulary(store, domain_id, name) to specify the domain.")
    end
...
"""
    ensure_vocabulary!(store::DataStore, domain_id::Int, vocab_name::String, description::String, items::AbstractVector{<:AbstractVocabularyItem})::Int

Creates or reuses a vocabulary by domain and name, and (re)loads items idempotently.
- 'store' is the DataStore object containing the database connection.
- 'domain_id' is the ID of the domain for which the vocabulary is being created.
...
"""
function ensure_vocabulary!(store::DataStore, domain_id::Int, vocab_name::String, description::String, items::AbstractVector{<:AbstractVocabularyItem})::Int
    ...
    df = DBInterface.execute(q_get, (domain_id, vocab_name)) |> DataFrame
    # Initialize to sentinel; we'll always assign in one of the branches
    vocab_id = -1
    if nrow(df) == 0
        vocab_id = insertwithidentity(db, "vocabularies",
            ["domain_id", "name", "description"], (domain_id, vocab_name, description))
    else
        vocab_id = df[1, :vocabulary_id]
        # Keep description up to date
        DBInterface.execute(db, raw"""UPDATE vocabularies SET description = $3 WHERE vocabulary_id = $1 AND domain_id = $2;""",
            (vocab_id, domain_id, description))
    end
...
"""
    ensure_vocabulary!(store::DataStore, vocab::Vocabulary) -> Int
...
function ensure_vocabulary!(store::DataStore, vocab::Vocabulary)::Int
    ...
    vocab.vocabulary_id = ensure_vocabulary!(store, vocab.domain_id, vocab.name, desc, vocab.items)
        ...
        if !ismissing(existing_variable_id)
            ...
            if var.value_type_id in (TRE_TYPE_CATEGORY, TRE_TYPE_MULTIRESPONSE)
                if !ismissing(var.vocabulary)
                    # Check if vocabulary already exists by domain and name
                    existing_vocab = get_vocabulary(store, var.domain_id, var.vocabulary.name)
                    if !isnothing(existing_vocab)
                        ...
                        # Create new vocabulary - ensure it has domain_id
                        var.vocabulary.domain_id = var.domain_id
...
function create_dataset_meta(store::DataStore, study::Study, dataset_name::String, description::String, datafile::DataFile)::DataSet
    ...
    @debug "Found $(nrow(eav_variables)) variables in EAV data"
    ...
    @debug "Found $(nrow(variables)) variables in EAV data for dataset $(dataset_name)"
...
function transform_eav_to_table!(store::DataStore, datafile::DataFile, dataset::DataSet; convert=true)::Nothing
    ...
    @debug "Conversion sql:\n $(sql * sql2)"
    DuckDB.query(store.lake, sql * sql2)
    @debug "Transformed EAV data from $(fpath) to table $(tbl)"
    return nothing
end
