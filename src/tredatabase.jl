...
function createstudies(conn::DBInterface.Connection)
    # Ensure the uuid-ossp extension (PostgreSQL 16+ may include uuidv7 in core; adjust if needed)
    try
        DBInterface.execute(conn, "CREATE EXTENSION IF NOT EXISTS \"uuid-ossp\";")
    catch e
        @warn "Could not create extension uuid-ossp (needed if uuidv7() not built-in)" exception=e
    end
    sql = raw"""
...
function createvariables(conn::DBInterface.Connection)
    ...
    COMMENT ON COLUMN domains.uri IS 'URI to the domain for public ontologies or namespaces';
    """
    DBInterface.execute(conn, sql)
