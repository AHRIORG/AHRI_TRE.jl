...
@info "TRE_DATABASE_PATH = $(ENV["TRE_DATABASE_PATH"])"
@info "TRE_DBNAME = $(ENV["TRE_DBNAME"])"
@info "DATA_INGEST_PATH = $(ENV["DATA_INGEST_PATH"])"
@info "DATA_DICTIONARY_PATH = $(ENV["DATA_DICTIONARY_PATH"])"
@info "ISO3_PATH = $(ENV["ISO3_PATH"])"
@info "TRE_LAKE_PATH = $(ENV["TRE_LAKE_PATH"])"
@info "TRE_LAKE_DB = $(ENV["TRE_LAKE_DB"])"
#endregion
...
if ingestCHAMPS
    ...
    ingest_source(source, ENV["TRE_DATABASE_PATH"], ENV["TRE_DBNAME"], ENV["DATA_INGEST_PATH"], ENV["ISO3_PATH"])

    @info "Ingesting CHAMPS dictionaries"
    ingest_dictionary(ingest, ENV["TRE_DATABASE_PATH"], ENV["TRE_DBNAME"], ENV["DATA_DICTIONARY_PATH"])

    @info "Ingesting CHAMPS deaths"
    ingestion_id = ingest_deaths(ingest, ENV["TRE_DATABASE_PATH"], ENV["TRE_DBNAME"], ENV["DATA_INGEST_PATH"])

    @info "Ingesting CHAMPS datasets"
    ingest_data(ingest, ENV["TRE_DATABASE_PATH"], ENV["TRE_DBNAME"], ENV["DATA_INGEST_PATH"]; ingestion_id=ingestion_id, sqlite=true, lake_data=ENV["TRE_LAKE_PATH"], lake_db=ENV["TRE_LAKE_DB"])
